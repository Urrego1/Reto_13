...
def imprimir_valores_ascendentes(diccionario):
    # Se obtienen los valores del diccionario los cuales deben ser del mismo tipo
    valores = diccionario.values()
    
    tipo_valor = type(next(iter(valores)))
    for valor in valores:
        if not isinstance(valor, tipo_valor):
            raise ValueError("Los valores del diccionario proporcionados, no son del mismo tipo.")
    
    #Se ordenan los valores
    valores_ordenados = sorted(diccionario.values())
    
    #Se imprimen los valores y ejemplo
...
def mezclar_diccionarios(dic1, dic2):
    #Se crea el diccionario_1
    nuevo_diccionario = dic1.copy() 

    #Se agregan las clases y valores del diccionario_2
    for clave, valor in dic2.items():
        # Verificar y agregar la clave solo si no está en el primer diccionario
        if clave not in nuevo_diccionario:
            nuevo_diccionario[clave] = valor

    return nuevo_diccionario

#Se muestran los diccionarios y se ejecuta
diccionario_1 = {
    'a': 1,
    'b': 2,
    'd': 5,
...
resultado = mezclar_diccionarios(diccionario_1, diccionario_2)
print(resultado)

```

**3.Cree un programa que lea de un archivo con dicho JSON y pueda imprimir los nombres completos (nombre y apellidos) de las personas que practican el deporte y que esten en un rango de edades dado por el usuario.:**
...
# Se añade esta función para evitar confusiones con tíldes u otros caracteres especiales. 
...
# Buscar e imprimir personas que practiquen un deporte
def buscar_por_deporte(datos, deporte):
    deporte = normalizar(deporte)  
    encontrados = []
    for persona in datos.values():
        deportes_normalizados = [normalizar(d) for d in persona['deportes']]
        if deporte in deportes_normalizados:
            nombre_completo = f"{persona['nombres']} {persona['apellidos']}"
            encontrados.append(nombre_completo)
    return encontrados

# Buscar e imprimir personas que cumplan ambas condiciones: deporte y rango de edades
def buscar_personas(datos, deporte, edad_min, edad_max):
    deporte = normalizar(deporte) 
    for persona in datos.values():
        deportes_normalizados = [normalizar(d) for d in persona['deportes']]
        if (deporte in deportes_normalizados and 
            edad_min <= persona['edad'] <= edad_max):
            nombre_completo = f"{persona['nombres']} {persona['apellidos']}"
            print(nombre_completo)

# Dadas las condiciones, se ejecuta el programa principal
if __name__ == "__main__":
    datos = cargar_datos('personas.json')

    # Se  solicita un deporte al usuario y se muestra si hay resultados
    deporte = input("Ingrese el deporte a buscar: ")
    
    personas_deporte = buscar_por_deporte(datos, deporte)
    if personas_deporte:
        print(f"Personas que practican {deporte.capitalize()}:")
        for persona in personas_deporte:
            print(persona)
    else:
        print(f"No se encontraron personas que practiquen {deporte.capitalize()}.")

    # Se solicita un rango de edades al usuario
    edad_min = int(input("Ingrese la edad mínima: "))
    edad_max = int(input("Ingrese la edad máxima: "))

    # Se muestran las personas que practiquen el deporte y estén en el rango de edades que el usuario solicitó
    print(f"Personas que practican {deporte.capitalize()} y tienen edades entre {edad_min} y {edad_max}:")
    buscar_personas(datos, deporte, edad_min, edad_max)
...
# Se cambia el formato para tener una fecha legible
...
# Inicia el proceso de revisar si hay alertas y que tipo es
for i in range(8):
    # Verificar alertas de precipitación
    if data['alertPrecip'][str(i)] != "-":
        fecha = convert_unix_to_date(data['dt'][str(i)])
        cantidad_lluvia = data['prcp'][str(i)]
        alertas['alertPrecip'].append({
            "fecha": fecha,
            "tipo": "Precipitación (mm)",
            "cantidad": cantidad_lluvia
        })

    # Verificar alertas de vientos
    if data['alertVelViento'][str(i)] != "-":
        fecha = convert_unix_to_date(data['dt'][str(i)])
        velocidad_viento = data['velViento'][str(i)]
        alertas['alertVelViento'].append({
            "fecha": fecha,
            "tipo": "Viento (m/s)",
            "velocidad": velocidad_viento
        })

    # Verificar alertas de temperatura máxima
    if data['alertTmpMax'][str(i)] != "-":
        fecha = convert_unix_to_date(data['dt'][str(i)])
        temperatura_max = data['tmpMax'][str(i)]
        alertas['alertTmpMax'].append({
            "fecha": fecha,
            "tipo": "Temperatura Máxima (°C)",
            "temperatura": temperatura_max
        })

    # Verificar alertas de temperatura mínima
    if data['alertTmpMin'][str(i)] != "-":
        fecha = convert_unix_to_date(data['dt'][str(i)])
        temperatura_min = data['tmpMin'][str(i)]
        alertas['alertTmpMin'].append({
            "fecha": fecha,
            "tipo": "Temperatura Mínima (°C)",
            "temperatura": temperatura_min 
        })

# Se imprimen las alertas
for tipo_alerta, eventos in alertas.items():
    for evento in eventos:
        print(f"Fecha: {evento['fecha']}, Tipo de Alerta: {evento['tipo']}, Detalles de la alerta: {evento.get('cantidad', evento.get('velocidad', evento.get('temperatura', '-')))}")
...
def obtener_imagen_perro():
    ...
    if response.status_code == 200:
        ...
        print("Imagen de un perro:")
        print(data['message'])
    else:
        print(f"Error: {response.status_code}")

def obtener_broma_aleatoria():
    # URL de la API
    ...
    if response.status_code == 200:
        ...
        # Extraer y mostrar la broma
        setup = data['setup']
        punchline = data['punchline']
        print("Broma aleatoria:")
        print(f"Setup: {setup}")
...
if __name__ == "__main__":
    ...
    obtener_imagen_perro()
    print()

    obtener_broma_aleatoria()
```
